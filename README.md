...
- **Issue time.** Hourly slots from 00:17 to 07:17 UTC, all before the 12:00 CET/CEST gate closure, and every other scheduled job issues the day's file too if it is missing. GitHub queues cron jobs under load and the delay runs to hours (day one, the 07:40 slot ran at 11:42; day two, every slot ran five hours late), so the window is wide and every attempt is a no-op once the file exists. Nothing is issued between 22:00 and 24:00 UTC, when the local day has already turned. Cron cannot follow daylight saving, so the cutoff is fixed in UTC and every file records `before_gate_closure`. A late file is published and flagged, and the watchdog alerts on it; a flagged file beats a hole.
